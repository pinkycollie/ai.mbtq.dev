...
export async function awardTokens(userId: string, type: string, amount: number) {
  try {
...
export async function saveConversation(userId: string, sessionId: string, messages: any[]) {
  try {
    await sql`
